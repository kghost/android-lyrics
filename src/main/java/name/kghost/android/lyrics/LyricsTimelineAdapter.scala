...
class LyricsTimelineAdapter(activity: Activity, timeline: Seq[(Int, Int, String)])
  extends BaseAdapter { adapter =>
  val mInflater = LayoutInflater.from(activity)
  val views = new Array[View](timeline.size)
  ...
  private def show(index: Int, dur: Int): Unit = views(index) match {
    case view: View => {
  ...
  private def find(time: Int, seq: Seq[(Int, Int, String)], index: Int, nearest: Option[Int], showing: List[(Int, Int)]): (Option[Int], List[(Int, Int)]) = seq match {
    case (start, end, string) :: rest =>
      find(time, rest, index + 1, nearest match {
        case Some(s) => if (time < start && start < s)
            Some(start)
          else
            nearest
        case None => if (time < start)
            Some(start)
          else
            None
  ...
  override def getView(position: Int, convertView: View, parent: ViewGroup): View =
    With(if (convertView == null)
      ...
      views(convertView.getTag.asInstanceOf[Int]) = null
      convertView
    }) { view =>
      view.setTag(position)
      var item = getItem(position)
      view.findViewById(R.id.TimelineText).asInstanceOf[TextView].setText(item._3)
      views(position) = With(view.findViewById(R.id.TimelineBar)) { _.clearAnimation }
      item match {
        case (start, end, _) => {
            val now = time_song
